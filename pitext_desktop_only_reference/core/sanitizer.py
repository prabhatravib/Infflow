...
class MermaidSanitizer:
    ...
    def sanitize(cls, snippet: str) -> str:
        ...
        text = cls._fix_flowchart_spacing(text)
        ...
        # STEP 6: Final validation - ensure flowchart spacing is correct
        text = cls._validate_flowchart_structure(text)
    ...
    def _sanitize_special_chars(cls, text: str) -> str:
        ...
        for line in lines:
            ...
            # Special handling for flowchart node lines - preserve indentation
            if stripped and stripped[0].isalpha() and '(' in stripped and ')' in stripped:
                # This looks like a node declaration (e.g., "A(Paris)" or 'B("text")')
                # Preserve the original line with indentation intact
                sanitized_lines.append(line)
                continue
    ...
    @staticmethod
    def _fix_flowchart_spacing(text: str) -> str:
        """
        Fix the specific issue where flowchart declarations are followed immediately by node IDs
        without proper spacing. This is a conservative fix that only addresses the known problem.
        """
        # Fix the specific case we're seeing: "flowchart TD" followed immediately by "A("
        # This is the minimal fix needed for the CORS diagram issue
        text = re.sub(r'(flowchart\s+(?:TD|TB|LR|RL))\s*(\w\()', r'\1\n    \2', text)
        text = re.sub(r'(graph\s+(?:TD|TB|LR|RL))\s*(\w\()', r'\1\n    \2', text)
        
        # Ensure all node declarations have proper indentation
        # This fixes the issue where subsequent nodes lose their indentation
        lines = text.splitlines()
        fixed_lines = []
        in_flowchart = False
        ...
        for line in lines:
            ...
            # Check if we're entering a flowchart section
            if stripped.startswith('flowchart') or stripped.startswith('graph'):
                in_flowchart = True
                fixed_lines.append(line)
                continue
            
            # If we're in a flowchart and this looks like a node declaration, ensure proper indentation
            if in_flowchart and stripped and stripped[0].isalpha() and '(' in stripped and ')' in stripped:
                # This is a node declaration - ensure it has proper indentation
            ...
            # If we hit a line that's not a node declaration, we might be out of the flowchart
            if in_flowchart and stripped and not (stripped[0].isalpha() and '(' in stripped and ')' in stripped):
                if stripped.startswith('style') or stripped.startswith('<') or stripped.startswith('>'):
                    # Still in flowchart (style or connection lines)
                    fixed_lines.append(line)
                    continue
                else:
                    # Might be out of flowchart section
                    in_flowchart = False
    ...
    @classmethod
    def _validate_flowchart_structure(cls, text: str) -> str:
        """
        Final validation step to ensure flowchart declarations have proper structure.
        This is a conservative fix that only addresses the specific issue we're seeing.
        """
        # Only fix the specific case: flowchart declaration followed immediately by node ID with parentheses
        # This is the minimal fix needed and won't interfere with valid Mermaid syntax
        text = re.sub(r'(flowchart\s+(?:TD|TB|LR|RL))\s*(\w\()', r'\1\n    \2', text)
        text = re.sub(r'(graph\s+(?:TD|TB|LR|RL))\s*(\w\()', r'\1\n    \2', text)
