...
class PiTextApp {
  ...
  async init() {
    ...
    try {
      ...
      // Show the initial placeholder in the diagram container
...
async handleAsk() {
  ...
  /**
   * Show the initial placeholder with the Textchart icon
   */
  showInitialPlaceholder() {
    const result = document.getElementById('result');
    if (result) {
      result.innerHTML = `
        <div class="placeholder" style="flex-direction: row; align-items: center; justify-content: center;">
          <img src="/desktop/assets/Textchart icon.png" alt="Textchart icon" class="placeholder-icon" style="width: 64px; height: 64px; margin-bottom: 0; margin-right: 0.5rem;" />
          <div style="font-size: 1.3rem; color: #fff; opacity: 0.7; margin-left: 0.2rem;">Textchart will appear here.</div>
        </div>
      `;
    }
  }
