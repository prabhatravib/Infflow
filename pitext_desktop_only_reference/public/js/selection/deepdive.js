...
// pitext-desktop/public/js/selection/deepdive.js
  ...
  _formatResponse(response) {
    // Normalize whitespace to avoid extra blank lines at top/bottom
    let normalized = typeof response === 'string' ? response : String(response || '');
    normalized = normalized.replace(/\r\n/g, '\n').trim();
    // Collapse runs of 3+ newlines to exactly two (one blank line between paragraphs)
    normalized = normalized.replace(/\n{3,}/g, '\n\n');

    // Escape HTML after normalization
    let formatted = this._escapeHtml(normalized);

    // Convert paragraphs - minimal spacing; mark first paragraph for CSS control
