...
export class SelectionHandler {
    ...
    tspans.forEach(tspan => {
      ...
      if (/^\d+[\.\)]\s/.test(text)) {
        ...
        tspan.addEventListener('click', (e) => {
          e.stopPropagation();
          const fullBulletContent = this._extractBulletPointContent(tspan, svg);
    ...
    texts.forEach(text => {
      ...
      if (/^\d+[\.\)]\s/.test(textContent)) {
        ...
        text.addEventListener('click', (e) => {
          e.stopPropagation();
          const fullBulletContent = this._extractBulletPointContent(text, svg);
  ...
  /**
   * Extract complete bullet point content
   * @private
   * @param {Element} clickedElement - The clicked text element
   * @param {SVGElement} svg - SVG container
   * @returns {string} Complete bullet point text
   */
  _extractBulletPointContent(clickedElement, svg) {
    ...
    const bulletTitle = bulletMatch[2]; // e.g., "Deep-fried Indian sweets"
