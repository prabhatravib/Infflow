...
export class DiagramGenerator {
  ...
  async generate(query, isRetry = false) {
      ...
      try {
          ...
          // Apply sequence-wide class for sequence comparison diagrams
...
async _renderDiagram(code, container) {
  ...
  _fixArrowVisibility(container) {
    setTimeout(() => {
      ...
      // Fix sequence diagram specific elements
      const sequenceElements = svg.querySelectorAll('.actor-line, .note rect, .actor rect');
      sequenceElements.forEach((el) => {
        if (el.tagName === 'rect') {
          el.setAttribute('stroke', '#000000');
          el.setAttribute('stroke-width', '2');
        } else if (el.classList.contains('actor-line')) {
          el.setAttribute('stroke', '#000000');
          el.setAttribute('stroke-width', '2');
        }
      });
